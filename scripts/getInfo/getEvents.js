...
async function main() {
  //   const provider = new ethers.getDefaultProvider(process.env.ALCHEMY_API_mainnet);
  const provider = new ethers.getDefaultProvider("mainnet");
  await getEvents(provider);
}

async function getEvents(provider) {
  const EnsContract = new ethers.Contract(EnsContractAddress, EnsAbi, provider);
  const filter = await EnsContract.filters.Transfer(
    null,
    null,
    "11122519784817965669626554097887055226770434951293697778703910953828570618213"
  );
  filter.fromBlock = 16344600;
  ...
  console.log("query time: ", (endTime - startTime) / 1000);
  console.log(logs.length);
  for (const log of logs) {qwe
    const res = await EnsContract.interface.parseLog(logs[0]);
    console.log(JSON.stringify(res, null, 2), log);
  }
