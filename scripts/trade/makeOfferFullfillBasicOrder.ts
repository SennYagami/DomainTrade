...
import { calculateOrderHash, getItemETH, getItem721, getItem20, getBasicOrderParameters } from "../utils/utils";
...
async function main() {
  const [offerMaker, accepter,royaltyHolder,platformWallet] = await hre.ethers.getSigners();
  ...
  const counter: BigNumber = await marketplaceContract.getCounter(offerMaker.address);
  ...
  const offer = [
    getItem20({
      ...
      startAmount: ethers.utils.parseEther("0.03"),
      endAmount: ethers.utils.parseEther("0.03"),
    }),
  ];

  //   construct consideration,
  //   consideration.slice(1) is payed to recipient
//     pay rest offerItem to order fullfiller
  const consideration = [
    getItem721({
      token: ensEthRegister[chainId],
      identifierOrCriteria: identifierOrCriteria,
      recipient: offerMaker.address,
    }),
    getItem20({
        token: wethAddress[chainId],
    ...
    getItem20({
    token: wethAddress[chainId],
    startAmount: ethers.utils.parseEther("0.005"),
    endAmount: ethers.utils.parseEther("0.005"),
...
//   const tx = await marketplaceContract.connect(accepter).fulfillBasicOrder(basicOrderParameters, {
//     value,
//   });

  console.log({ maker: offerMaker.address, accepter: accepter.address });
  console.log({
